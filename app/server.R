...
function(input, output) {
  model <- reactive({
    ...
    model$shocks <- list(
      e = matrix(gherm$nodes, nrow = 1) * sqrt(2),
      w = matrix(gherm$weights / sum(gherm$weights), ncol = 1)
    )
    model$P <- model$params$pbar *
      (1 +
        (model$s - model$params$dbar) /
          (model$params$elastD * model$params$dbar))
  ...
  output$demandFun <- renderPlot({
    ...
    dt <- tibble(
      A = m$s,
      Pinv = m$params$pbar *
        (1 + (A - m$params$dbar) / (m$params$elastD * m$params$dbar)),
      P = m$P
    )

    dt |>
      pivot_longer(cols = -A, names_to = "Variable", values_to = "value") |>
  ...
  output$PriceDistrib <- renderPlot({
    ...
    ggplot(data = tibble(P = c(sim$P)), aes(x = P)) +
      ...
      ggtitle(
        "Price distribution (in black price distribution without stocks, in blue mean price)"
      ) +
      theme_light()
  })
  ...
  output$statTable <- renderTable({
    ...
    tibble(P = c(m$sim$P), A = c(m$sim$A)) |>
      ...
      mutate(AC1 = c(A_acf[1], P_acf[1]), AC2 = c(A_acf[2], P_acf[2])) |>
      add_row(

  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      sliderInput(inputId = "SDe",
                  ...
                  value = 2,
                  round = -1),
      sliderInput(inputId = "pbar",
                  label = "Deterministic steady-state price, \\(\\bar{P}\\)",
                  min = 0.5,
                  max = 10,
                  value = 1,
                  round = -1),
      sliderInput(inputId = "dbar",
                  label = "Deterministic steady-state quantity, \\(\\bar{D}\\)",
                  min = 0.5,
                  max = 10,
                  value = 1,
                  round = -1),
      ...
      h2("Model definition"),
      helpText("$$\\text{Consumption: } D(P_t) = \\bar{D}\\left(\\frac{P_t}{\\bar{P}}\\right)^{\\alpha^D}$$"),
      helpText("$$\\text{Availability: } A_t = S_{t-1}+\\bar{D}\\exp\\left(\\sigma_{e} e_t\\right)$$"),

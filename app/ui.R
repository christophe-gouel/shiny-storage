...
fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      h2("Model definition"),
      helpText(
        "$$\\text{Consumption: } D(P_t) = \\bar{D}\\left[1+\\alpha^{D}\\left(\\frac{P_t-\\bar{P}}{\\bar{P}}\\right)\\right]$$"
      ),
      helpText(
        "$$\\text{Availability: } A_t = S_{t-1}+\\bar{D}\\left(1+\\sigma_{e} e_t\\right)$$"
      ),
      helpText(
        "$$\\text{Storage: } \\beta E_t P_{t+1}-P_t-k\\bar{P}\\le 0, =0 \\text{ if } S_t >0$$"
      ),
      helpText("$$\\text{Market clearing: } A_t = D\\left(P_t\\right)+S_t$$"),
      h3("Fixed parameters"),
      helpText(
        "Steady-state quantities and price are normalized to unity: \\(\\bar{D}=\\bar{P}=1\\)"
      ),
      helpText("$$\\beta=1/\\left(1+r\\right)=0.98$$"),
      helpText("$$e\\sim N\\left(0,1\\right)$$"),
      h2("Warnings"),
      helpText(
        "The model may not converge for all possible combinations of parameters. If this is the case, try other values."
      )
    ),
